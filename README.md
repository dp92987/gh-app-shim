...
By default every Claude Code session acts as the bot. For some repos you may
want Claude to act as **you** instead — e.g. your personal repos or dotfiles.
List them in `config.env` and the shim passes through to the real `gh` with
your own auth there, exactly like a normal terminal:
...
Both are space-separated and case-insensitive; orgs match the repo owner, repos
are `owner/repo`. Leave them unset and every Claude Code call acts as the bot
(original behavior).
...
way `gh` itself picks a repo. Commands with no detectable repo (e.g.
`gh api user`, `gh auth status`) act as the bot.

> This setting only chooses *which credential* to use. Whether the bot can
> touch a repo **at all** is governed by the App's own GitHub permissions and
> installed-repo list — manage access there, not here.
